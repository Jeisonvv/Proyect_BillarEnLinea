...
type WompiActivityReturnResponse = {
  ok: boolean;
  data?: WompiActivityReturnData;
};
...
export const metadata: Metadata = {
  title: "Retorno de pago de actividad",
  ...
  switch (status) {
    case "APPROVED":
      return {
        ...
        description: "Tu pago fue aprobado. Tus números ya están registrados para el sorteo.",
    ...
    default:
      return {
        ...
        description: "La transacción no quedó aprobada. Puedes intentarlo de nuevo si los números siguen disponibles.",
        tone: "border-rose-400/28 bg-rose-950/24 text-rose-50",
      };
  }
}

async function getReturnData(reference?: string): Promise<WompiActivityReturnData | null> {
  if (!reference) return null;
  try {
    const response = await getJson<WompiActivityReturnResponse>(
      `/api/payments/wompi/activities/return?reference=${encodeURIComponent(reference)}`,
      { cache: "no-store" },
    );
...
export default async function WompiActivityReturnPage({ searchParams }: PageProps) {
    ...
    <main className="mx-auto grid min-h-screen w-full max-w-6xl gap-6 px-4 py-8 sm:px-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(22rem,0.95fr)] xl:px-10">
        ...
        <div className="relative h-72 overflow-hidden border-b border-white/10 bg-black/28 sm:h-88">
          {data?.activity.imageUrl ? (
            <Image
              ...
              alt={`Imagen de la actividad ${data.activity.name}`}
              fill
              priority
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
            ...
            <h1 className="mt-4 text-3xl font-semibold leading-tight text-white sm:text-4xl xl:text-5xl">
              {data?.activity.name ?? "Estamos verificando tu participación"}
            </h1>
        ...
        <div className="grid gap-6 p-5 sm:p-7 xl:p-8">
          ...
          {data ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <article className="rounded-[1.45rem] border border-white/8 bg-white/5 p-4 text-white/86">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.24em] text-white/44">Valor pagado</p>
                <p className="mt-3 text-sm">{formatMoney(data.payment.amountInCents / 100)}</p>
              </article>
              <article className="rounded-[1.45rem] border border-white/8 bg-white/5 p-4 text-white/86">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.24em] text-white/44">Sorteo</p>
                <p className="mt-3 text-sm leading-7">{formatLongDate(data.activity.drawDate)}</p>
              </article>
              <article className="rounded-[1.45rem] border border-white/8 bg-white/5 p-4 text-white/86">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.24em] text-white/44">Ticket</p>
                <p className="mt-3 text-sm">{humanizeToken(data.ticket.status)}</p>
              </article>
            </div>
          ) : (
          ...
          {/* Números comprados */}
          {data && data.ticket.numbers.length > 0 ? (
            <div className="rounded-[1.6rem] border border-white/8 bg-white/4 p-5">
              <p className="font-mono text-[0.68rem] uppercase tracking-[0.26em] text-white/46">Tus números</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {data.ticket.numbers.map((num) => (
                  <span
                    key={num}
                    className="rounded-xl border border-[rgba(246,196,79,0.28)] bg-[rgba(246,196,79,0.12)] px-3 py-1.5 font-mono text-sm font-semibold text-[rgba(255,233,174,0.96)]"
                  >
                    {num}
                  </span>
                ))}
      ...
      <aside className="grid gap-4 self-start">
        <section className="rounded-[1.9rem] border border-white/10 bg-[linear-gradient(180deg,rgba(246,196,79,0.08),rgba(255,255,255,0.03))] p-5 text-white">
          <p className="font-mono text-[0.7rem] uppercase tracking-[0.28em] text-[rgba(246,196,79,0.76)]">Actividad</p>
          <h2 className="mt-4 text-2xl font-semibold">{data?.activity.name ?? "Pendiente de confirmación"}</h2>
          {data ? (
            <div className="mt-4 grid gap-3">
              <div className="rounded-[1.2rem] border border-white/8 bg-black/18 p-4">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.22em] text-white/44">Premio</p>
                <p className="mt-2 text-sm font-semibold text-white">{data.activity.prize}</p>
              </div>
              <div className="rounded-[1.2rem] border border-white/8 bg-black/18 p-4">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.22em] text-white/44">Precio por número</p>
                <p className="mt-2 text-sm font-semibold text-white">{formatMoney(data.activity.ticketPrice)}</p>
              </div>
              <div className="rounded-[1.2rem] border border-white/8 bg-black/18 p-4">
                <p className="font-mono text-[0.66rem] uppercase tracking-[0.22em] text-white/44">Total comprado</p>
                <p className="mt-2 text-sm font-semibold text-white">{formatMoney(data.ticket.total)}</p>
              </div>
            </div>
          ) : null}
        </section>
        ...
        <section className="grid gap-3 rounded-[1.9rem] border border-white/8 bg-white/4 p-5">
          ...
          {data?.activity.slug ? (
            <Link
              href={`/home/activities/${data.activity.slug}`}
