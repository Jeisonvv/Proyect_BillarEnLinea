...
function isShellVisible(pathname: string) {
    ...
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-white/8 bg-[rgba(7,9,12,0.82)] backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-10">
          ...
          <nav className="hidden items-center gap-6 text-sm text-white/68 lg:flex" aria-label="Navegacion principal">
            {navItems.map((item) => (
              ...
              <nav className="grid gap-2" aria-label="Navegacion movil">
                {navItems.map((item, index) => (
                  <Link
                    ...
                    className="translate-x-3 rounded-2xl px-4 py-3 text-sm font-medium text-white/78 opacity-0 transition duration-300 ease-out hover:bg-white/8 hover:text-white group-open:translate-x-0 group-open:opacity-100"
