...
type HomeState = {
  loading: boolean;
  torneos: Awaited<ReturnType<typeof getLandingTournaments>>;
  eventos: Awaited<ReturnType<typeof getLandingEvents>>;
  activities: Awaited<ReturnType<typeof getLandingActivities>>;
  noticias: Awaited<ReturnType<typeof getLandingPosts>>;
  productos: Awaited<ReturnType<typeof getLandingProducts>>;
};

const emptyState = { items: [], total: 0, error: null };

export function HomeBrowserPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([torneos, eventos, activities, noticias, productos]) => {
        if (!active) return;
        setState({
          loading: false,
          torneos,
    ...
    <main className="grid gap-6">
      ...
      >
        <div className="grid gap-4">
          {state.loading ? (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">Cargando torneos...</div>
          ) : state.torneos.items.length > 0 ? state.torneos.items.map((torneo, index) => (
            ...
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">
              {state.torneos.error ? `No fue posible cargar los torneos. ${state.torneos.error}` : "No hay torneos disponibles por ahora."}
            </div>
          )}
        </div>
      </HomeSection>
      ...
      >
        <div className="grid gap-4">
          {state.loading ? (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">Cargando actividades...</div>
          ) : state.activities.items.length > 0 ? state.activities.items.map((activity) => (
            <ActivityCard key={activity.id} item={activity} />
          )) : (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">
              {state.activities.error ? `No fue posible cargar las actividades. ${state.activities.error}` : "No hay actividades activas en este momento."}
            </div>
          )}
        </div>
      </HomeSection>
      ...
      >
        <div className="grid gap-4">
          {state.loading ? (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">Cargando productos...</div>
          ) : state.productos.items.length > 0 ? state.productos.items.map((producto) => (
            <ProductCard key={producto.id} item={producto} />
          )) : (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">
              {state.productos.error ? `No fue posible cargar los productos. ${state.productos.error}` : "Aun no hay productos visibles en la tienda."}
            </div>
          )}
        </div>
      </HomeSection>
      ...
      >
        <div className="grid gap-4">
          {state.loading ? (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">Cargando eventos...</div>
          ) : state.eventos.items.length > 0 ? state.eventos.items.map((evento) => (
            <EventCard key={evento.id} item={evento} />
          )) : (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">
              {state.eventos.error ? `No fue posible cargar los eventos. ${state.eventos.error}` : "Aun no hay eventos publicados para mostrar en esta seccion."}
            </div>
          )}
        </div>
      </HomeSection>
      ...
      >
        <div className="grid gap-4">
          {state.loading ? (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">Cargando noticias...</div>
          ) : state.noticias.items.length > 0 ? state.noticias.items.map((post) => (
            <PostCard key={post.id} item={post} />
          )) : (
            <div className="rounded-3xl border border-dashed border-white/10 bg-black/12 px-5 py-6 text-sm leading-7 text-white/62">
              {state.noticias.error ? `No fue posible cargar las noticias. ${state.noticias.error}` : "No hay noticias publicadas por ahora."}
            </div>
          )}
        </div>
      </HomeSection>
