...
export function StoreBrowserPage() {
  const [state, setState] = useState<Awaited<ReturnType<typeof getLandingProducts>> & { loading: boolean }>({
    loading: true,
    items: [],
    total: 0,
    error: null,
  ...
  useEffect(() => {
    ...
    void getLandingProducts(60)
      ...
      .catch((error) => {
        if (!active) return;
        setState({ loading: false, items: [], total: 0, error: error instanceof Error ? error.message : "No fue posible cargar la tienda." });
      });
  ...
  const featuredByCategory = new Map<ProductCategory, (typeof state.items)[number]>();
        ...
        {state.loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {categoriesWithProducts.map((category) => {
              ...
              return (
                <Link key={category} href={href} className="group flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/5 transition hover:border-[rgba(246,196,79,0.45)] hover:bg-white/8">
                  <div className="relative aspect-4/3 w-full overflow-hidden bg-black/40">
                    {featured?.image ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center text-sm text-white/46">Sin referencia aún</div>
                    )}
                  ...
                  <div className="flex flex-1 flex-col gap-2 p-4">
                    ...
                    {featured ? (
                      <>
                        ...
                        {featured.brand ? <p className="text-xs uppercase tracking-[0.16em] text-white/52">{featured.brand}</p> : null}
                    ...
                    <span className="mt-auto inline-flex items-center gap-1 pt-2 text-sm font-medium text-[#f6c44f] transition group-hover:translate-x-1">Ver categoría →</span>
                  </div>
                </Link>
              );
            })}
          ...
          <section className="rounded-3xl border border-dashed border-white/12 bg-white/3 p-6 text-sm leading-7 text-white/64">
            {state.error ? `No fue posible cargar la tienda. ${state.error}` : "Aún no hay productos visibles en la tienda."}
          </section>
        )}
      </section>
    </main>
