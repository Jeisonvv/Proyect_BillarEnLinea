...
/**
 * Boletos de actividad comprados por un usuario en una transacción.
 *
 * Un ActivityTicket representa UNA COMPRA, que puede incluir
 * uno o varios boletos a la vez.
 *
 * Ejemplo: Juan compra 5 boletos de la actividad "Taco Predator":
 *   {
 *     activity: ObjectId("actividad-predator"),
 *     user: ObjectId("juan"),
 ...
 *   }
 *
 * Para saber los boletos de Juan en esa actividad:
 *   await ActivityTicket.findNumbersByUserAndActivity(juan._id, actividad._id)
 *   // → ["14", "27", "53", "61", "88"]
 *
...
export interface IActivityTicket {
  activity: mongoose.Types.ObjectId; // A qué actividad pertenecen estos boletos
...
activityTicketSchema.pre("validate", async function () {
  ...
  const activityDoc = await Activity.findById(this.activity).select("ticketPrice totalTickets");
  if (!activityDoc) {
    throw new Error("La actividad no existe.");
  }

  const normalizedNumbers = this.numbers.map((numberValue) => normalizeActivityNumberInput(numberValue, activityDoc.totalTickets));
  ...
  if (activityDoc.ticketPrice === 0) {
    ...
    if (normalizedNumbers.length !== 1) {
      throw new Error("En una actividad gratuita cada usuario solo puede obtener un número.");
    }

    if (this.status !== TicketStatus.PAID) {
      throw new Error("Los boletos de actividades gratuitas deben confirmarse inmediatamente.");
    }
  ...
  this.unitPrice = activityDoc.ticketPrice;
...
// Para ver todas las compras de un usuario en una actividad específica
activityTicketSchema.index({ activity: 1, user: 1 });

// Para verificar si un número ya fue vendido antes de asignarlo.
// Si alguien intenta comprar el número 14 de la actividad X, buscan en este índice.
activityTicketSchema.index({ activity: 1, numbers: 1 });
...
activityTicketSchema.index(
  { activity: 1, participantIdentityDocument: 1 },
  {
    unique: true,
    name: "free_activity_identity_once_per_activity",
...
activityTicketSchema.index({ activity: 1, isWinner: 1 });
...
/**
 * Devuelve todos los boletos comprados por un usuario en todas las actividades.
 * Hace populate de la actividad para incluir nombre, premio, fecha del sorteo, etc.
 *
 * Ejemplo:
 *   const tickets = await ActivityTicket.findByUser(user._id);
 *   tickets[0].activity  // → objeto Activity completo (nombre, premio, etc.)
...
activityTicketSchema.statics.findByUser = function (
  ...
  return this.find({ user: userId })
    .populate("activity") // Trae el objeto Activity completo
    .sort({ createdAt: -1 }); // Más recientes primero
};

/**
 * Devuelve todos los boletos PAGADOS de una actividad específica.
 * Útil para el admin al momento de realizar el sorteo:
 * ver quién participó y con qué números.
 *
 * Ejemplo:
 *   const tickets = await ActivityTicket.findByActivity(actividad._id);
...
/**
 * Devuelve TODOS los números que tiene un usuario en una actividad específica.
 * Agrupa todas sus compras y aplana los arrays de numbers en uno solo.
 *
 * Ejemplo:
 *   // Juan hizo 2 compras: [14, 27] y después [53, 61]
 *   const nums = await ActivityTicket.findNumbersByUserAndActivity(juan._id, actividad._id);
